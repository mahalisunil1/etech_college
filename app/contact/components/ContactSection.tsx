          ...
          <div className="lg:col-span-5 flex flex-col gap-8 reveal">
                    ...
                    {item.title === "Email" ? (
                      ...
                      <div className="text-slate-600 text-sm leading-relaxed font-semibold">
                        {item.desc.split(", ").map((num, i) => (
                          <span key={i} className="block sm:inline sm:mr-3 last:mr-0">
                            <a href={`tel:${num.replace(/\s+/g, "")}`} className="hover:text-blue-600 hover:underline">
                              {num}
                            </a>
                            {i < item.desc.split(", ").length - 1 && <span className="hidden sm:inline text-slate-300 ml-3">|</span>}
                          </span>
                        ))}
                      </div>
                    ) : (
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mt-6 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4">
                <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white shadow-sm border border-blue-100 transition-all duration-300 group">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z" />
                  </svg>
                </a>
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white shadow-sm border border-rose-100 transition-all duration-300 group">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-12 h-12 rounded-xl bg-red-50 text-red-600 hover:bg-red-600 hover:text-white shadow-sm border border-red-100 transition-all duration-300 group">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
